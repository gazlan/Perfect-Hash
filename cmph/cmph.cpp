...
const char* cmph_names[]   =
{
   "bmz", "bmz8", "chm", "brz", "fch", "bdz", "bdz_ph", "chd_ph", "chd", NULL
};

struct cmph_vector_t
{
   void* vector;
   DWORD position; // access position when data is a vector
};
...
typedef struct
{
   ...
   DWORD                      struct_size;         /* The size of the struct */
   DWORD                      key_offset;          /* The byte offset of the key in the struct */
   DWORD                      key_len;             /* The length of the key */
} cmph_struct_vector_t;


static cmph_io_adapter_t*  cmph_io_vector_new(void* vector,DWORD nkeys);
static void                cmph_io_vector_destroy(cmph_io_adapter_t* key_source);

static cmph_io_adapter_t*  cmph_io_struct_vector_new(void* vector,DWORD struct_size,DWORD key_offset,DWORD key_len,DWORD nkeys);
static void                cmph_io_struct_vector_destroy(cmph_io_adapter_t* key_source);

static int key_nlfile_read(void* data,char** key,DWORD* keylen)
{
   FILE* fd = (FILE*) data;
   *key = NULL;
   *keylen = 0;
...
static int key_byte_vector_read(void* data,char** key,DWORD* keylen)
{
   cmph_vector_t* cmph_vector = (cmph_vector_t*) data;

   BYTE**         keys_vd     = (BYTE**) cmph_vector->vector;
...
static int key_struct_vector_read(void* data,char** key,DWORD* keylen)
{
...
static int key_vector_read(void* data,char** key,DWORD* keylen)
{
   cmph_vector_t* cmph_vector = (cmph_vector_t*) data;
   ...
   *keylen = (DWORD) strlen(keys_vd[cmph_vector->position]);
...
static void key_nlfile_dispose(void* data,char* key,DWORD keylen)
{
   free(key);
}

static void key_vector_dispose(void* data,char* key,DWORD keylen)
{
   free(key);
}
...
static DWORD count_nlfile_keys(FILE* fd)
{
   DWORD count = 0;
...
void cmph_config_destroy(cmph_config_t* mph)
{
   if (mph)
   {
      TRACE("Destroying mph with algo %s\n",cmph_names[mph->algo]);
...
cmph_t* cmph_new(cmph_config_t* mph)
{
   ...
   TRACE("Creating mph with algorithm %s\n",cmph_names[mph->algo]);

   switch (mph->algo)
   {
      case CMPH_CHM:
         TRACE("Creating chm hash\n");

         mphf = chm_new(mph,c);
         break;
      case CMPH_BMZ:
         /* included -- Fabiano */
         TRACE("Creating bmz hash\n");

         mphf = bmz_new(mph,c);
         break;
      case CMPH_BMZ8:
         /* included -- Fabiano */
         TRACE("Creating bmz8 hash\n");

         mphf = bmz8_new(mph,c);
         break;
      case CMPH_BRZ:
         /* included -- Fabiano */
         TRACE("Creating brz hash\n");
      ...
      case CMPH_FCH:
         /* included -- Fabiano */
         TRACE("Creating fch hash\n");

         mphf = fch_new(mph,c);
         break;
      case CMPH_BDZ:
         /* included -- Fabiano */
         TRACE("Creating bdz hash\n");

         mphf = bdz_new(mph,c);
         break;
      case CMPH_BDZ_PH:
         /* included -- Fabiano */
         TRACE("Creating bdz_ph hash\n");

         mphf = bdz_ph_new(mph,c);
         break;
      case CMPH_CHD_PH:
         /* included -- Fabiano */
         TRACE("Creating chd_ph hash\n");

         mphf = chd_ph_new(mph,c);
         break;
      case CMPH_CHD:
         /* included -- Fabiano */
         TRACE("Creating chd hash\n");
...
cmph_t* cmph_load(FILE* f)
{
   ...
   TRACE("Loading mphf generic parts\n");
   ...
   TRACE("Loading mphf algorithm dependent parts\n");
   ...
   switch (mphf->algo)
   {
      ...
      case CMPH_BMZ:
         /* included -- Fabiano */
         TRACE("Loading bmz algorithm dependent parts\n");

         bmz_load(f,mphf);
         break;
      case CMPH_BMZ8:
         /* included -- Fabiano */
         TRACE("Loading bmz8 algorithm dependent parts\n");

         bmz8_load(f,mphf);
         break;
      case CMPH_BRZ:
         /* included -- Fabiano */
         TRACE("Loading brz algorithm dependent parts\n");

         brz_load(f,mphf);
         break;
      case CMPH_FCH:
         /* included -- Fabiano */
         TRACE("Loading fch algorithm dependent parts\n");

         fch_load(f,mphf);
         break;
      case CMPH_BDZ:
         /* included -- Fabiano */
         TRACE("Loading bdz algorithm dependent parts\n");

         bdz_load(f,mphf);
         break;
      case CMPH_BDZ_PH:
         /* included -- Fabiano */
         TRACE("Loading bdz_ph algorithm dependent parts\n");

         bdz_ph_load(f,mphf);
         break;
      case CMPH_CHD_PH:
         /* included -- Fabiano */
         TRACE("Loading chd_ph algorithm dependent parts\n");

         chd_ph_load(f,mphf);
         break;
      case CMPH_CHD:
         /* included -- Fabiano */
         TRACE("Loading chd algorithm dependent parts\n");
