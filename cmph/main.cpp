...
int   getopt(int argc,char** const argv,char* opstring);
...
int main(int argc,char** argv)
{
   ...
   DWORD                memory_availability  = 0;
   DWORD                b                    = 0;
   DWORD                keys_per_bin         = 1;
   ...
   while (true)
   {
      ...
      switch (ch)
      {
         case 's':
            {
               char* cptr;
               seed = (DWORD) strtoul(optarg,&cptr,10);
               if (*cptr != 0)
               {
         ...
         case 'k':
            {
               char* endptr;
               nkeys = (DWORD) strtoul(optarg,&endptr,10);
               if (*endptr != 0)
               {
         ...
         case 'M':
            {
               char* cptr;
               memory_availability = (DWORD) strtoul(optarg,&cptr,10);
               if (*cptr != 0)
               {
         ...
         case 'b':
            {
               char* cptr;
               b = (DWORD) strtoul(optarg,&cptr,10);
               if (*cptr != 0)
               {
         ...
         case 't':
            {
               char* cptr;
               keys_per_bin = (DWORD) strtoul(optarg,&cptr,10);
               if (*cptr != 0)
               {
   ...
   if (generate)
   {
      ...
      cmph_config_set_tmp_dir(config,(BYTE *) tmp_dir);
   ...
   else
   {
      ...
      DWORD siz   = cmph_size(mphf);
      hashtable = (BYTE *) calloc(siz,sizeof(BYTE));
